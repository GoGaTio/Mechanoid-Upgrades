...
namespace MU
{
	public static class MechUpgradeUtility
	{
		...
		[DebugAction("Pawns", "Upgrade Mech", false, false, false, false, 0, false, actionType = DebugActionType.ToolMapForPawns, allowedGameStates = AllowedGameStates.PlayingOnMap, requiresBiotech = true)]
		...
		public static string DescWithStats(MU.MechUpgrade upgrade)
		{
			...
			if(upgrade.def.allowRemoteControl || upgrade.def.commandRange != null || !upgrade.def.statFactors.NullOrEmpty() || !upgrade.def.statOffsets.NullOrEmpty() || upgrade.def.ability != null || (!upgrade.comps.NullOrEmpty() && upgrade.comps.Any((UpgradeComp c)=> !c.ExtraDescStrings().EnumerableNullOrEmpty())))
            {
				sb.AppendLine();
            }
            else
			...
			foreach(UpgradeComp c in upgrade.comps)
            {
		...
		public static void DEV_ReloadAll(Thing t)
		{
			...
			foreach (MechUpgrade u in comp.upgrades)
			{
				foreach(UpgradeCompReloadable c in u.GetListOfComp<UpgradeCompReloadable>())
                {
                    if (c.NeedsReload)
                    {

...
namespace MU
{
	public class MechanoidUpgrades : Mod
	{
		public static Harmony harmonyInstance;

		public MechanoidUpgrades(ModContentPack content)
			: base(content)
		{
			harmonyInstance = new Harmony("MechanoidUpgradesPatch");
	...
	public class Patch_QualityOffset
	{
		...
		public static void Postfix(Pawn pawn, SkillDef relevantSkill, ref QualityCategory __result)
		{
			...
			foreach(MechUpgrade upgrade in comp.upgrades.Where((MechUpgrade u) => u.GetComp<UpgradeComp_QualityOffset>() != null))
            {
				__result = (QualityCategory)Mathf.Min((int)__result + upgrade.GetComp<UpgradeComp_QualityOffset>().Offset, 6);
			}
		}
	}

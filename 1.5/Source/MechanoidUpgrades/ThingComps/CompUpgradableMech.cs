	...
	public class CompUpgradableMechanoid : ThingComp
	{
		...
		public override float GetStatFactor(StatDef stat)
		{
			...
			foreach (MU.MechUpgrade u in upgrades)
			{
				...
				foreach (UpgradeComp c in u.comps)
				{
		...
		public bool NeedsReload
		{
			get
			{
				...
				if (upgrades.Any((MechUpgrade u) => !u.GetListOfComp<UpgradeCompReloadable>().NullOrEmpty() && u.GetListOfComp<UpgradeCompReloadable>().Any((UpgradeCompReloadable c) => c.NeedsReload)))
				{
					return true;
				}
				return false;
		...
		private bool HasAnyReloadable
		{
			get
			{
				...
				if (upgrades.Any((MechUpgrade u) => !u.comps.NullOrEmpty() && !u.GetListOfComp<UpgradeCompReloadable>().NullOrEmpty()))
				{
					return true;
				}
				return false;
		...
		public override void PostExposeData()
		{
			...
			if (Scribe.mode == LoadSaveMode.PostLoadInit && !upgrades.NullOrEmpty())
			{
				...
				{
					MechUpgrade ru = upgrades.FirstOrDefault((MechUpgrade u) => u.def == upgradeForReload);
